...
class Checkout extends StatefulWidget {
  ...
  const Checkout({super.key, required this.selectedItems, required this.totalAmount});
...
class _CheckoutState extends State<Checkout> {
  ...
  Future<void> _loadBuyer() async {
    try {
      ...
      final row = await _supabase
          .from('user_data')
          .select('user_name, user_account, user_delivery_address')
          .eq('user_id', user.id)
          .maybeSingle();
      if (row == null) return;
      _buyer = Map<String, dynamic>.from(row);
      ...
      if (parsed.isNotEmpty) {
        initial = parsed.firstWhere((a) => a['is_default'] == true, orElse: () => parsed.first);
      }
      if (mounted) {
        setState(() {
  ...
  Future<void> _loadSeller() async {
    try {
      ...
      final ids = widget.selectedItems
          .map((r) => _itemOf(r))
          .map((it) => it['item_seller_id']?.toString() ?? it['seller_id']?.toString())
          .where((s) => s != null && s.isNotEmpty)
          .toSet();
      if (ids.length > 1) {
        _blockReason = "Your cart contains items from multiple shops. Please checkout one shop at a time.";
        setState(() => _loadingSeller = false);
        return;
      }
      final first = widget.selectedItems.isNotEmpty ? _itemOf(widget.selectedItems.first) : <String, dynamic>{};
      _sellerId = first['item_seller_id']?.toString() ?? first['seller_id']?.toString();
      ...
      final row = await _supabase
          .from('sellers')
          .select('seller_id, seller_store_name, seller_store_address, seller_payment_method, seller_preferences, seller_delivery_rates')
      ...
      if (row != null) {
        ...
        _sellerRates = rawRates is List
            ? rawRates.whereType<Map>().map((e) => Map<String, dynamic>.from(e)).toList()
            : <Map<String, dynamic>>[];
        ...
        if (ff is List) _sellerFulfillment = ff.map((e) => e.toString()).toList();
  ...
  Map<String, dynamic>? _matchedRate() {
    ...
    for (final r in _sellerRates) {
      if (_norm(r['rate_municipality']) == am && _norm(r['rate_barangay']) == ab) return r;
    }
    return null;
  }
  ...
  num get _minOrder => (_sellerPrefs['delivery_min_order'] as num?)?.toDouble() ?? 0;

  bool get _belowMinOrder => _isDelivery && _minOrder > 0 && _itemSubtotal() < _minOrder;

  // Buyer's chosen address has no matching seller delivery rate.
  bool get _isUnreachable => _isDelivery && _selectedAddress != null && _matchedRate() == null;

  num get _effectiveDeliveryFee => (_isDelivery && !_isUnreachable) ? (_deliveryFee() ?? 0) : 0;

  bool get _requiresProof => _paymentMethod == _GCASH || _paymentMethod == _MAYA;
  ...
  Future<String?> _uploadProof() async {
    ...
    try {
      ...
      await _supabase.storage.from('order_proof').uploadBinary(path, bytes,
  ...
  String? _mobileNumberForSelected() {
    String? raw;
    if (_paymentMethod == _GCASH) raw = _sellerPrefs['gcash_number']?.toString();
  ...
  String? _qrUrlForSelected() {
    if (_paymentMethod == _GCASH) return _sellerPrefs['gcash_qr_image']?.toString();
    if (_paymentMethod == _MAYA) return _sellerPrefs['maya_qr_image']?.toString();
    return null;
  }
  ...
  Map<String, dynamic> _itemOf(Map<String, dynamic> raw) {
    if (raw['store_items'] is Map<String, dynamic>) return Map<String, dynamic>.from(raw['store_items']);
    return raw;
  }
  ...
  Future<void> _processCheckout() async {
    if (_blockReason != null) {
      SnackbarMessenger().showSnackbar(context, SnackbarMessenger.failed, _blockReason!);
      return;
    }
    if (_hasUnavailableItem()) {
      SnackbarMessenger().showSnackbar(context, SnackbarMessenger.failed, "One or more items are no longer available.");
      return;
    }
    if (_paymentMethod == null) {
      SnackbarMessenger().showSnackbar(context, SnackbarMessenger.failed, "Please select a payment method.");
      return;
    }
    if (_isPaymentFirst && (_paymentMethod == _COD || _paymentMethod == _OTC)) {
      SnackbarMessenger().showSnackbar(context, SnackbarMessenger.failed, "This shop requires online payment. Please choose GCash or Maya.");
      return;
    }
    if ((_paymentMethod == _GCASH || _paymentMethod == _MAYA) && (_qrUrlForSelected() == null || _qrUrlForSelected()!.isEmpty)) {
      SnackbarMessenger().showSnackbar(context, SnackbarMessenger.failed, "Seller hasn't uploaded a $_paymentMethod QR. Please pick another method or contact the seller.");
      return;
    }
    if (_requiresProof && _proofFile == null && (_proofUrl == null || _proofUrl!.isEmpty)) {
      SnackbarMessenger().showSnackbar(context, SnackbarMessenger.failed, "Please upload your payment proof receipt.");
      return;
    }
    if (_selectedAddress == null) {
      SnackbarMessenger().showSnackbar(context, SnackbarMessenger.failed, "Please add a delivery address to continue.");
      return;
    }
    if (_fulfillment == "pickup" && _sellerAddressLine().isEmpty) {
      SnackbarMessenger().showSnackbar(context, SnackbarMessenger.failed, "Pickup address unavailable. Please contact the seller.");
      return;
    }
    if (_isUnreachable) {
      SnackbarMessenger().showSnackbar(context, SnackbarMessenger.failed, "This shop doesn't deliver to your selected address. Choose another address or pick up at the store.");
      return;
    }
    ...
    try {
      ...
      for (final raw in widget.selectedItems) {
        ...
        if (raw['cart_id'] != null) {
          await _supabase.from('cart').delete().eq('cart_id', raw['cart_id']);
        }
      }
        ...
        Navigator.pushReplacement(
          ...
          MaterialPageRoute(
            builder: (_) => OrderPlaced(
              ...
              deliveryType: deliveryAddress['delivery_type']?.toString() ?? _fulfillment,
      ...
      if (mounted) {
        _loadingDialog.dismiss();
        SnackbarMessenger().showSnackbar(context, SnackbarMessenger.failed, "Checkout failed: $e");
      }
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: _loadingSeller
          ...
          : _blockReason != null
              ...
              : LayoutBuilder(builder: (context, constraints) {
                  ...
                  int step = 0;
                  String nextStep() { step += 1; return "$step"; }
                  return Center(
      ...
      bottomNavigationBar: _loadingSeller || _blockReason != null ? null : _buildPlaceOrderBar(total),
    );
  }
  ...
  Widget _buildStepHeader() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [primaryBlue.withValues(alpha: 0.08), themeOrange.withValues(alpha: 0.05)]),
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text("Ordering from", style: TextStyle(color: textSecondary, fontWeight: FontWeight.w700, fontSize: 11.5)),
                Text(storeName, style: TextStyle(color: primaryDark, fontWeight: FontWeight.w900, fontSize: 15)),
              ],
            ),
          ),
          if (_isPaymentFirst)
            Container(
              ...
              decoration: BoxDecoration(color: themeOrange.withValues(alpha: 0.12), borderRadius: BorderRadius.circular(10)),
              child: Text("PAY FIRST", style: TextStyle(color: themeOrange, fontWeight: FontWeight.w900, fontSize: 10)),
            ),
        ],
      ),
    );
      ...
      children: [
        if (step != null) ...[
          Container(
            ...
            decoration: BoxDecoration(color: themeOrange, shape: BoxShape.circle),
            child: Text(step, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 11)),
          ),
        ...
        Text(label, style: TextStyle(fontWeight: FontWeight.w900, color: primaryDark, fontSize: 14, letterSpacing: 0.3)),
      ],
    );
  }
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Icon(Icons.remove_shopping_cart_rounded, size: 64, color: textSecondary),
            const SizedBox(height: 12),
            Text("Can't checkout yet", style: TextStyle(fontWeight: FontWeight.w900, color: primaryDark, fontSize: 18)),
            const SizedBox(height: 8),
            Text(message, textAlign: TextAlign.center, style: TextStyle(color: textSecondary, fontSize: 13.5, height: 1.5)),
  ...
  Widget _buildFulfillmentCard({String stepLabel = "1"}) {
    return _card(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _sectionTitle(Icons.local_shipping_rounded, "HOW TO RECEIVE", step: stepLabel),
          const SizedBox(height: 10),
          Row(
            children: [
              Expanded(child: _fulfillmentChip("delivery", Icons.local_shipping_rounded, "Delivery", "Ship to my address")),
              const SizedBox(width: 8),
              Expanded(child: _fulfillmentChip("pickup", Icons.storefront_rounded, "Pickup", "At the store")),
            ],
          ),
        ],
      ),
  ...
  Widget _fulfillmentChip(String value, IconData icon, String title, String subtitle) {
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(12),
          border: Border.all(color: selected ? themeOrange : cardBorder, width: selected ? 1.5 : 1),
        ),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            ...
            Text(title, style: TextStyle(fontWeight: FontWeight.w900, color: primaryDark, fontSize: 13.5)),
            Text(subtitle, style: TextStyle(color: textSecondary, fontSize: 11.5)),
          ],
        ),
      ),
    );
  ...
  Widget _buildAddressOrPickupCard({String stepLabel = "2"}) {
    if (_fulfillment == "pickup") {
      ...
      return _card(
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            _sectionTitle(Icons.location_on_rounded, "PICKUP LOCATION", step: stepLabel),
            const SizedBox(height: 10),
            Container(
              padding: const EdgeInsets.all(12),
              decoration: BoxDecoration(
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(_seller['seller_store_name']?.toString() ?? "Store", style: TextStyle(fontWeight: FontWeight.w900, color: primaryDark, fontSize: 13.5)),
                        const SizedBox(height: 3),
                        Text(addr.isEmpty ? "Address not provided" : addr, style: TextStyle(color: textSecondary, fontSize: 12.5, height: 1.4)),
                      ],
                    ),
                  ),
                ],
    ...
    return _card(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _sectionTitle(Icons.location_on_rounded, "DELIVERY ADDRESS", step: stepLabel),
          const SizedBox(height: 10),
        ...
        Row(children: [
          Expanded(
            child: OutlinedButton.icon(
              ...
              icon: Icon(Icons.swap_horiz_rounded, color: themeOrange, size: 16),
              label: Text(
                _savedAddresses.length > 1 ? "Change Address" : "Choose Address",
                style: TextStyle(color: themeOrange, fontWeight: FontWeight.w900, fontSize: 12),
              ),
          ...
          Expanded(
            child: OutlinedButton.icon(
              ...
              icon: Icon(Icons.add_location_alt_rounded, color: primaryBlue, size: 16),
              label: Text("Add / Manage", style: TextStyle(color: primaryBlue, fontWeight: FontWeight.w900, fontSize: 12)),
              style: OutlinedButton.styleFrom(
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(10),
          border: Border.all(color: isSelected ? themeOrange : cardBorder, width: isSelected ? 1.5 : 1),
        ),
        child: Row(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
                  ...
                  Row(children: [
                    Flexible(
                      child: Text(
                        ...
                        style: TextStyle(fontWeight: FontWeight.w900, color: primaryDark, fontSize: 13.5),
                      ),
                    ),
                    if (isDefault) ...[
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(color: accentGreen, borderRadius: BorderRadius.circular(6)),
                        child: const Text("DEFAULT", style: TextStyle(color: Colors.white, fontSize: 8.5, fontWeight: FontWeight.w900, letterSpacing: 0.6)),
                      ),
                    ],
                  ]),
                  const SizedBox(height: 3),
                  Text(line, style: TextStyle(color: textSecondary, fontSize: 12, height: 1.4)),
                ],
              ),
            ),
          ],
  ...
  Widget _buildUnreachableBanner() {
    return Container(
      ...
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Icon(Icons.local_shipping_rounded, color: Color(0xFFB91C1C), size: 18),
          const SizedBox(width: 8),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text("Unreachable address",
                    style: TextStyle(color: Color(0xFFB91C1C), fontWeight: FontWeight.w900, fontSize: 12.5)),
                const SizedBox(height: 2),
                Text(
                  "This shop doesn't deliver to your selected barangay. Choose another address or pick up at the store.",
                  style: const TextStyle(color: Color(0xFFB91C1C), fontSize: 11.5, height: 1.35, fontWeight: FontWeight.w600),
                ),
              ],
            ),
          ),
        ...
        children: [
          Row(children: [
            ...
            Text("No saved delivery address",
                style: TextStyle(fontWeight: FontWeight.w900, color: primaryDark, fontSize: 13)),
          ]),
          const SizedBox(height: 6),
          Text("Add an address so we can deliver your order. You can manage your addresses here.",
                  ...
                  itemBuilder: (_, i) {
                    ...
                    return Padding(
                      ...
                      child: InkWell(
                        ...
                        child: _buildAddressOption(a, a['id'] == _selectedAddress?['id']),
                      ),
                    );
                  },
                ),
        ...
        children: [
          Row(children: [
            ...
            Text("BUYER INFORMATION",
                style: TextStyle(color: primaryBlue, fontWeight: FontWeight.w900, fontSize: 11, letterSpacing: 0.8)),
          ]),
          const SizedBox(height: 8),
          _buyerKv(Icons.badge_outlined, "Name", fullName.isEmpty ? "—" : fullName),
          if (middle.isNotEmpty) _buyerKv(Icons.short_text_rounded, "Middle name", middle),
          if ((selected['contact_number'] ?? '').toString().trim().isNotEmpty)
            _buyerKv(Icons.call_rounded, "Contact", (selected['contact_number']).toString().trim()),
          if ((selected['email'] ?? '').toString().trim().isNotEmpty)
            _buyerKv(Icons.alternate_email_rounded, "Email", (selected['email']).toString().trim()),
          if (notes.isNotEmpty) _buyerKv(Icons.notes_rounded, "Address notes", notes),
        ],
      ),
    );
  }
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          SizedBox(
            width: 92,
            child: Text(label, style: TextStyle(color: textSecondary, fontSize: 11.5, fontWeight: FontWeight.w700)),
          ),
          Expanded(
            child: Text(value, style: TextStyle(color: primaryDark, fontSize: 12.5, fontWeight: FontWeight.w700)),
          ),
        ],
      ),
    );
  ...
  Widget _buildItemsCard() {
    return _card(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _sectionTitle(Icons.shopping_bag_rounded, "ORDER ITEMS (${widget.selectedItems.length})"),
          const SizedBox(height: 10),
          for (int i = 0; i < widget.selectedItems.length; i++) ...[
            _buildItemRow(widget.selectedItems[i]),
            if (i < widget.selectedItems.length - 1) Divider(color: cardBorder, height: 18),
          ],
        ],
      ),
    );
      ...
      children: [
        ...
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(name, maxLines: 2, overflow: TextOverflow.ellipsis, style: TextStyle(fontWeight: FontWeight.w700, color: primaryDark, fontSize: 13.5)),
              if (variation != null) ...[
                const SizedBox(height: 3),
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
              ...
              Text("₱${Utility().formatPrice(unitPrice)} each", style: TextStyle(color: textSecondary, fontSize: 11.5, fontWeight: FontWeight.w600)),
              const SizedBox(height: 6),
              _qtyStepper(raw, qty.toInt(), stocks?.toInt()),
            ],
          ),
        ),
        Text("₱${Utility().formatPrice(lineTotal)}", style: TextStyle(color: themeOrange, fontWeight: FontWeight.w900, fontSize: 14)),
      ],
    );
  }
      ...
      child: Row(
        ...
        children: [
          ...
          Container(
            ...
            alignment: Alignment.center,
            child: Text("$qty", style: TextStyle(fontWeight: FontWeight.w900, color: primaryDark, fontSize: 13)),
          ),
  ...
  Widget _qtyButton(IconData icon, bool enabled, VoidCallback onTap) {
    return InkWell(
      ...
      child: Padding(
        padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
        child: Icon(icon, size: 16, color: enabled ? primaryDark : textSecondary.withValues(alpha: 0.4)),
      ),
    );
  }

  Widget _imgFallback() => Container(color: bgColor, child: Icon(Icons.image_outlined, color: textSecondary.withValues(alpha: 0.4)));

  IconData _paymentIcon(String m) {
    switch (m) {
      case _GCASH: return Icons.account_balance_wallet_rounded;
      case _MAYA: return Icons.credit_card_rounded;
      case _COD: return Icons.local_shipping_rounded;
      case _OTC: return Icons.point_of_sale_rounded;
  ...
  String _paymentSubtitle(String m) {
    switch (m) {
      case _GCASH: return "Scan the seller's QR & send";
      case _MAYA: return "Scan the seller's QR & send";
      case _COD: return "Pay cash when the order arrives";
      case _OTC: return "Pay with cash at the store counter";
      default: return "";
    }
  }
  ...
  Widget _buildPaymentCard({String stepLabel = "3"}) {
    ...
    final needsQrButMissing = (_paymentMethod == _GCASH || _paymentMethod == _MAYA) && (qrUrl == null || qrUrl.isEmpty);
    return _card(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _sectionTitle(Icons.payments_rounded, "PAYMENT METHOD", step: stepLabel),
          const SizedBox(height: 4),
          Text(
            _isPaymentFirst
          ...
          if (available.isEmpty)
            Container(
              ...
              child: Text("No payment methods available for this option.", style: TextStyle(color: Colors.red.shade700, fontWeight: FontWeight.w700, fontSize: 12.5)),
            )
          else
            ...available.map((m) => Padding(
                  padding: const EdgeInsets.only(bottom: 8),
                  child: _paymentOption(m, _paymentIcon(m), _paymentLabel(m), _paymentSubtitle(m)),
                )),
          if ((_paymentMethod == _GCASH || _paymentMethod == _MAYA) && _mobileNumberForSelected() != null) ...[
            const SizedBox(height: 8),
            _buildWalletNumberPanel(_paymentMethod!, _mobileNumberForSelected()!),
          ],
          ...
          if (needsQrButMissing) ...[
            ...
            Container(
              ...
              decoration: BoxDecoration(color: Colors.amber.withValues(alpha: 0.12), borderRadius: BorderRadius.circular(10), border: Border.all(color: Colors.amber.withValues(alpha: 0.5))),
              child: Row(children: [
                Icon(Icons.warning_amber_rounded, color: Colors.amber.shade800, size: 18),
                const SizedBox(width: 8),
                Expanded(child: Text("Seller hasn't uploaded a $_paymentMethod QR yet. Pick another method or message the seller.", style: TextStyle(color: Colors.amber.shade900, fontSize: 12, fontWeight: FontWeight.w600))),
              ]),
            ),
          ],
        ],
          ...
          Row(
            children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(color: themeOrange.withValues(alpha: 0.15), borderRadius: BorderRadius.circular(10)),
                child: Text("REQUIRED", style: TextStyle(color: themeOrange, fontWeight: FontWeight.w900, fontSize: 9.5, letterSpacing: 0.5)),
              ),
            ],
          ),
          const SizedBox(height: 6),
          Text("Send the total via $_paymentMethod, then upload a screenshot of your receipt below.",
              style: TextStyle(color: textSecondary, fontSize: 12, height: 1.4)),
          const SizedBox(height: 10),
          if (hasFile)
            Row(
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(_proofFile!.name, style: TextStyle(color: primaryDark, fontWeight: FontWeight.w700, fontSize: 12), maxLines: 2, overflow: TextOverflow.ellipsis),
                      const SizedBox(height: 6),
                      Row(children: [
                        OutlinedButton.icon(
                          onPressed: _pickProof,
                          icon: Icon(Icons.swap_horiz_rounded, size: 16, color: primaryBlue),
                          label: Text("Replace", style: TextStyle(color: primaryBlue, fontWeight: FontWeight.w800, fontSize: 12)),
                          style: OutlinedButton.styleFrom(side: BorderSide(color: primaryBlue), padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6)),
                        ),
                        const SizedBox(width: 8),
                        TextButton.icon(
                          onPressed: () => setState(() { _proofFile = null; _proofUrl = null; }),
                          icon: Icon(Icons.delete_outline_rounded, size: 16, color: Colors.red),
                          label: Text("Remove", style: TextStyle(color: Colors.red, fontWeight: FontWeight.w800, fontSize: 12)),
                        ),
                      ]),
                    ],
                  ),
            ...
            InkWell(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: Colors.white,
                  borderRadius: BorderRadius.circular(10),
                  border: Border.all(color: themeOrange.withValues(alpha: 0.4), style: BorderStyle.solid),
                ),
                child: Column(
                  children: [
                    Icon(Icons.cloud_upload_rounded, color: themeOrange, size: 28),
                    const SizedBox(height: 6),
                    Text("Tap to upload receipt", style: TextStyle(color: themeOrange, fontWeight: FontWeight.w900, fontSize: 13)),
                    const SizedBox(height: 2),
                    Text("PNG / JPG", style: TextStyle(color: textSecondary, fontSize: 11)),
                  ],
                ),
              ),
            ),
                  ...
                  child: Image.network(
                    ...
                    errorBuilder: (_, __, ___) => Container(
                      ...
                      child: Column(mainAxisSize: MainAxisSize.min, children: [
                        Icon(Icons.broken_image_rounded, color: textSecondary, size: 48),
                        const SizedBox(height: 8),
            ...
            Positioned(
              ...
              child: Container(
                ...
                child: Row(children: [
                  ...
                  const SizedBox(width: 6),
                  Expanded(child: Text("Pinch to zoom. Scan with your $_paymentMethod app.", style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w700))),
                ]),
              ),
            ),
          ],
      ...
      child: Row(
        children: [
          Container(
            ...
            decoration: BoxDecoration(color: primaryBlue.withValues(alpha: 0.12), borderRadius: BorderRadius.circular(8)),
            child: Icon(Icons.phone_iphone_rounded, color: primaryBlue, size: 18),
          ),
          const SizedBox(width: 10),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text("$method Mobile Number", style: TextStyle(color: textSecondary, fontSize: 10.5, fontWeight: FontWeight.w800, letterSpacing: 0.8)),
                const SizedBox(height: 2),
                SelectableText(
                  number,
                  style: TextStyle(color: primaryDark, fontWeight: FontWeight.w900, fontSize: 15, letterSpacing: 0.4),
                ),
              ],
            ),
          ),
          IconButton(
            ...
            onPressed: () async {
              ...
              if (!mounted) return;
              SnackbarMessenger().showSnackbar(context, SnackbarMessenger.success, "$method number copied");
            },
        ...
        children: [
            ...
            children: [
              InkWell(
                ...
                child: Stack(children: [
                  ClipRRect(
                    borderRadius: BorderRadius.circular(8),
                    child: Image.network(url, width: 100, height: 100, fit: BoxFit.cover, errorBuilder: (_, __, ___) => Container(width: 100, height: 100, color: bgColor, child: Icon(Icons.qr_code_2_rounded, color: textSecondary))),
                  ),
                  Positioned(
                    right: 4, bottom: 4,
                    child: Container(
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text("Pay via $_paymentMethod", style: TextStyle(fontWeight: FontWeight.w900, color: primaryDark, fontSize: 13)),
                    const SizedBox(height: 4),
                    Text("1. Tap the QR to view full screen\n2. Scan & send the total amount\n3. Upload the payment proof below",
          ...
          SizedBox(
            ...
            child: OutlinedButton.icon(
              ...
              label: Text("View QR Code", style: TextStyle(color: primaryBlue, fontWeight: FontWeight.w900, fontSize: 13)),
              style: OutlinedButton.styleFrom(
          ...
          children: [
            ...
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(title, style: TextStyle(fontWeight: FontWeight.w800, color: primaryDark, fontSize: 13.5)),
                  if (subtitle.isNotEmpty) Text(subtitle, style: TextStyle(color: textSecondary, fontSize: 11.5)),
                ],
              ),
            ),
            Icon(selected ? Icons.radio_button_checked_rounded : Icons.radio_button_unchecked_rounded, color: selected ? themeOrange : textSecondary, size: 20),
          ],
        ),
      ),
    );
        ...
        children: [
          ...
          TextField(
            ...
            decoration: InputDecoration(
              ...
              focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(10), borderSide: BorderSide(color: themeOrange, width: 1.5)),
            ),
          ),
        ],
      ),
        ...
        children: [
          ...
          if (_isDelivery)
            Padding(
              ...
              child: Row(
                children: [
                  Text("Delivery Fee", style: TextStyle(color: textSecondary, fontSize: 13, fontWeight: FontWeight.w600)),
                  const Spacer(),
                  _isUnreachable
                      ? Text("Unreachable", style: TextStyle(color: Colors.red.shade700, fontWeight: FontWeight.w900, fontSize: 13))
                      : Text("₱${Utility().formatPrice(_effectiveDeliveryFee)}", style: TextStyle(color: primaryDark, fontWeight: FontWeight.w800, fontSize: 13)),
                ],
              ),
            ),
          Row(
            children: [
              Text("Payment", style: TextStyle(color: textSecondary, fontSize: 13, fontWeight: FontWeight.w600)),
              const Spacer(),
              Text(_paymentMethod ?? "—", style: TextStyle(color: primaryDark, fontWeight: FontWeight.w800, fontSize: 13)),
            ],
          ),
          ...
          Row(
            ...
            children: [
              Text("Total", style: TextStyle(fontWeight: FontWeight.w900, color: primaryDark, fontSize: 16)),
              Text("₱${Utility().formatPrice(total)}", style: TextStyle(fontWeight: FontWeight.w900, color: themeOrange, fontSize: 18)),
            ],
          ),
        ],
      ),
  ...
  Widget _summaryRow(String label, num value, {String? note}) {
    return Row(
      children: [
        ...
        if (note != null) ...[
          const SizedBox(width: 6),
          Flexible(child: Text("($note)", style: TextStyle(color: textSecondary.withValues(alpha: 0.7), fontSize: 11), overflow: TextOverflow.ellipsis)),
        ],
        const Spacer(),
        Text("₱${Utility().formatPrice(value)}", style: TextStyle(color: primaryDark, fontWeight: FontWeight.w800, fontSize: 13)),
      ],
    );
  }
  ...
  Widget _buildPlaceOrderBar(num total) {
    ...
    final disabled = _isProcessing || _paymentMethod == null || unavailable || missingAddress || unreachable || belowMin;
    final unavailableNames = unavailable ? _unavailableItemNames() : const <String>[];
    return SafeArea(
      child: Container(
            ...
            if (!unavailable && missingAddress) ...[
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(10),
                  border: Border.all(color: themeOrange.withValues(alpha: 0.45)),
                ),
                child: Row(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                      ...
                      child: Column(
                        ...
                        children: [
                          Text("Delivery address required",
                              style: TextStyle(color: themeOrange, fontWeight: FontWeight.w900, fontSize: 12.5)),
                          const SizedBox(height: 2),
                          Text(
                            "Add an address in My Account → Delivery Address to place your order.",
                            style: TextStyle(color: primaryDark.withValues(alpha: 0.75), fontSize: 11.5, height: 1.35, fontWeight: FontWeight.w600),
                          ),
                        ],
                      ),
                    ),
            ...
            if (!unavailable && !missingAddress && belowMin) ...[
              Container(
                ...
                child: Row(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Icon(Icons.shopping_bag_rounded, color: themeOrange, size: 18),
                    const SizedBox(width: 8),
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text("Minimum order not met",
                              style: TextStyle(color: themeOrange, fontWeight: FontWeight.w900, fontSize: 12.5)),
                          const SizedBox(height: 2),
                          Text(
            ...
            Row(
              children: [
                Column(
                  ...
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    Text("TOTAL", style: TextStyle(color: textSecondary, fontWeight: FontWeight.w800, fontSize: 10, letterSpacing: 1)),
                    Text("₱${Utility().formatPrice(total)}", style: TextStyle(color: themeOrange, fontWeight: FontWeight.w900, fontSize: 20)),
                  ],
                ),
