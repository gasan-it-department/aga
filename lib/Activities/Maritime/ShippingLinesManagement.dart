...
class ShippingLinesManagement extends StatefulWidget {
  ...
  @override
  State<ShippingLinesManagement> createState() => _ShippingLinesManagementState();
}
...
class _ShippingLinesManagementState extends State<ShippingLinesManagement> {
  ...
  Future<void> _fetchShippingLines({bool isRefresh = false}) async {
    ...
    try {
      var query = supabase.from('shipping_lines').select();
      ...
      final shippingLineData = await query
          .order('shipping_line_added_date', ascending: true)
  ...
  Future<void> _loadMoreShippingLines() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _shippingLines.addAll(List<Map<String, dynamic>>.from(data));
  ...
  Future<void> _fetchPorts() async {
    try {
      final portData = await supabase.from('ports').select().order('port_name', ascending: true);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
          ...
          elevation: 0,
          title: const Text("Shipping Lines", style: TextStyle(fontWeight: FontWeight.w900, letterSpacing: -0.5)),
          actions: [
            IconButton(
              ...
              onPressed: () async {
                if (_ports.isEmpty || _ports.length < 2) {
                  SnackbarMessenger().showSnackbar(context, SnackbarMessenger.neutral, "Please add at least 2 ports first.");
                  return;
                }

                final dynamic result = await Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => AddEditShippingLine(ports: _ports)),
                );
                ...
                Expanded(
                  child: _isLoading
                    ...
                    child: ListView.builder(
                      ...
                      itemCount: _shippingLines.length + (_isLoadingMore ? 1 : 0),
  ...
  Widget _buildLineCard(Map<String, dynamic> line) {
    final List schedules = line['shipping_line_schedules'] is String ? jsonDecode(line['shipping_line_schedules']) : (line['shipping_line_schedules'] ?? []);
    final List fares = line['shipping_line_fares'] is String ? jsonDecode(line['shipping_line_fares']) : (line['shipping_line_fares'] ?? []);

    return Container(
      margin: const EdgeInsets.only(bottom: 12),
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16), border: Border.all(color: outlineColor)),
      child: Material(
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(line['shipping_line_name'] ?? "Unknown Line", style: TextStyle(fontWeight: FontWeight.bold, color: textPrimary, fontSize: 16)),
                      const SizedBox(height: 4),
                      Text("${schedules.length} Routes • ${fares.length} Fares", style: TextStyle(color: textSecondary, fontSize: 13)),
                    ],
                  ),
                ),
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(line['shipping_line_name'] ?? "Options",
              ...
              _buildSheetItem(
                  ...
                  onTap: () async {
                    ...
                    final dynamic result = await Navigator.push(
                      context,
                      MaterialPageRoute(builder: (context) => AddEditShippingLine(shippingLine: line, ports: _ports)),
                    );
                    ...
                    if (result != null && result is Map<String, dynamic>) {
                      if (_searchQuery.isNotEmpty) {
                        ...
                        setState(() {
                          final index = _shippingLines.indexWhere((l) => l['shipping_line_id'] == line['shipping_line_id']);
                          if (index != -1) {
                            _shippingLines[index] = { ..._shippingLines[index], ...result };
                          }
                        });
                      }
                    } else if (result == true) {
              ...
              _buildSheetItem(
                  ...
                  onTap: () {
                    Navigator.pop(context);
                    Navigator.push(
                        context, MaterialPageRoute(builder: (context) => VesselsManagement(shippingLine: line)));
                  }
              ),
  ...
  void _deleteLine(Map<String, dynamic> line) {
    ...
    _classicDialog.setMessage("Permanently remove ${line['shipping_line_name']} and ALL its registered vessels?\n\nThis action cannot be undone.");
    _classicDialog.setNegativeMessage("CANCEL");
    _classicDialog.setPositiveMessage("DELETE");
    _classicDialog.setCancelable(true);

    if (mounted) {
      _classicDialog.showTwoButtonDialog(context, (negativeClicked) {
        _classicDialog.dismissDialog();
        ...
        try {
          await supabase.from('vessels').delete().eq('shipping_line_id', line['shipping_line_id']);
          await supabase.from('shipping_lines').delete().eq('shipping_line_id', line['shipping_line_id']);
          ...
          String lineName = line['shipping_line_name'].toString().toUpperCase();
          ...
          await MaritimeActivityLogger.createLog(
              ...
              message: "$lineName and its fleet were permanently deleted by [$assignedPort] - $userName.",
              creatorId: userId
          );
          ...
          setState(() {
            _shippingLines.removeWhere((l) => l['shipping_line_id'] == line['shipping_line_id']);
          });

          if (mounted) {
            SnackbarMessenger().showSnackbar(context, SnackbarMessenger.success, "${line['shipping_line_name']} and its fleet were deleted.");
          }
        } catch (e) {
          _loadingDialog.dismiss();
      ...
      children: [
        ...
        Center(
          child: Column(
            ...
            children: [
              Icon(
                  _searchQuery.isNotEmpty ? Icons.search_off_rounded : Icons.anchor_rounded,
                  size: 60,
                  color: outlineColor
              ),
              const SizedBox(height: 16),
              Text(
                  _searchQuery.isNotEmpty ? "No matches found" : "No records found",
