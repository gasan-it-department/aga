...
class _MaritimeUserViewState extends State<MaritimeUserView> {
  ...
  // Operational metrics
  int get _activeVoyagesCount => _vessels.where((v) => (v['vessel_status']?['status'] ?? '').toString().toLowerCase() == 'departed').length;
  int get _dockedCount => _vessels.where((v) => ['docked', 'arrived'].contains((v['vessel_status']?['status'] ?? '').toString().toLowerCase())).length;
  int get _onboardingCount => _vessels.where((v) => (v['vessel_status']?['status'] ?? '').toString().toLowerCase() == 'onboarding').length;
  ...
  Future<void> _fetchData() async {
    ...
    try {
      final responses = await Future.wait([
        ...
        _supabase.from('vessels').select('*, shipping_lines(shipping_line_name)').order('vessel_name'),
        _supabase.from('shipping_lines').select('shipping_line_id, shipping_line_name').order('shipping_line_name'),
      ]);
      ...
      if (mounted) {
        setState(() {
          ...
          _shippingLines = List<Map<String, dynamic>>.from(responses[2] as List);
          _isLoading = false;
        });
      }
    } catch (e) {
  ...
  List<Map<String, dynamic>> get _filteredVessels {
    return _vessels.where((vessel) {
      ...
      if (_searchQuery.isNotEmpty) {
        ...
        if (!name.contains(_searchQuery.toLowerCase()) && !type.contains(_searchQuery.toLowerCase())) {
          return false;
        }
      }
      ...
      if (_selectedPortId != null) {
        ...
        final matchesPort = currentPort == _selectedPortId ||
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text(
          ...
          style: TextStyle(fontWeight: FontWeight.w900, letterSpacing: 0.5, fontSize: 15, color: Color(0xFF0F172A)),
        ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  value,
                  style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w900, color: Color(0xFF0F172A), height: 1.1),
                ),
        ...
        children: [
          ...
          Padding(
            ...
            child: TextField(
              ...
              style: TextStyle(fontSize: 13, color: primaryDark, fontWeight: FontWeight.bold),
                  ...
                  child: Container(
                    ...
                    child: DropdownButtonHideUnderline(
                      child: DropdownButton<String>(
                        ...
                        hint: const Row(
                          children: [
                            Icon(Icons.place_rounded, size: 15, color: Color(0xFF64748B)),
                            SizedBox(width: 4),
                            Text("All Ports", style: TextStyle(fontSize: 12, color: Color(0xFF64748B), fontWeight: FontWeight.w600)),
                          ],
                        ),
                        icon: const Icon(Icons.arrow_drop_down_rounded, color: Color(0xFF64748B)),
                        isExpanded: true,
                        style: const TextStyle(fontSize: 12, color: Color(0xFF0F2042), fontWeight: FontWeight.w600),
                        items: [
                          ...
                          ..._ports.map((port) {
                            return DropdownMenuItem<String>(
                              ...
                              child: Text(port['port_name']?.toString() ?? "Port"),
                            );
                          }),
                        ],
                        onChanged: (val) {
                  ...
                  child: Container(
                    ...
                    child: DropdownButtonHideUnderline(
                      child: DropdownButton<String>(
                        ...
                        hint: const Row(
                          children: [
                            Icon(Icons.directions_boat_rounded, size: 15, color: Color(0xFF64748B)),
                            SizedBox(width: 4),
                            Text("All Lines", style: TextStyle(fontSize: 12, color: Color(0xFF64748B), fontWeight: FontWeight.w600)),
                          ],
                        ),
                        icon: const Icon(Icons.arrow_drop_down_rounded, color: Color(0xFF64748B)),
                        isExpanded: true,
                        ...
                        items: [
                          ...
                          ..._shippingLines.map((line) {
                            return DropdownMenuItem<String>(
                              ...
                              child: Text(line['shipping_line_name']?.toString() ?? "Line"),
                            );
                          }),
                        ],
                        onChanged: (val) {
  ...
  Widget _buildVesselCard(Map<String, dynamic> vessel) {
    ...
    final shippingLineName = vessel['shipping_lines']?['shipping_line_name'] ?? 'Unknown Shipping Line';
    ...
    if (displayStatus.toLowerCase() == 'departed' && departed > 0) {
      final etaEpoch = departed + (travelDuration * 60 * 1000);
    ...
    } else if (displayStatus.toLowerCase() == 'onboarding' && onboardingTime > 0) {
      final onboardingTimeStr = Utility().formatEpochToTime(onboardingTime);
      timeDetail = "Onboarding at: $onboardingTimeStr";
            ...
            child: Column(
              ...
              children: [
                  ...
                  child: Row(
                    children: [
                      Container(
                        ...
                        child: Icon(Icons.directions_boat_rounded, color: statusColor, size: 20),
                      ),
                ...
                Padding(
                  ...
                  child: isTransit
                      ? _buildRouteTrack(originName, destName, progress, statusColor)
                      : Row(
                          children: [
                            Container(
                              ...
                              child: const Icon(Icons.anchor_rounded, size: 14, color: Color(0xFF475569)),
                            ),
                            const SizedBox(width: 8),
                            Expanded(
                              child: Column(
                ...
                Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      Row(
                        children: [
                          const Icon(Icons.airplane_ticket_outlined, size: 14, color: Color(0xFF64748B)),
  ...
  Widget _buildRouteTrack(String origin, String destination, double progress, Color statusColor) {
          ...
          children: [
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text("ORIGIN PORT", style: TextStyle(fontSize: 8.5, fontWeight: FontWeight.w900, color: Color(0xFF94A3B8))),
                  const SizedBox(height: 2),
                  Text(
                    origin,
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  const Text("DESTINATION PORT", style: TextStyle(fontSize: 8.5, fontWeight: FontWeight.w900, color: Color(0xFF94A3B8))),
                  const SizedBox(height: 2),
                  Text(
                    destination,
                    style: const TextStyle(fontSize: 12.5, fontWeight: FontWeight.w800, color: Color(0xFF1E293B)),
            ...
            Align(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: [
                    BoxShadow(color: statusColor.withValues(alpha: 0.15), blurRadius: 4, spreadRadius: 1),
                  ],
                ),
                child: Icon(Icons.directions_boat_rounded, color: statusColor, size: 11),
              ),
            ),
          ],
        ),
