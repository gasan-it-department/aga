...
class _MdrrmoAdministratorState extends State<MdrrmoAdministrator> {
  ...
  Future<void> _fetchAnalytics(int zipCode) async {
    try {
      // 1. Personnel
      final personnelResponse = await _supabase.from('mdrrmo_personnels').select('personnel_id').eq('personnel_municipality', zipCode);
      
      // 2. Units
      final unitsResponse = await _supabase.from('vehicles').select('vehicle_id, vehicle_status').eq('vehicle_municipal_owner', zipCode);
      final List allUnits = unitsResponse as List;
      int dispatchedCount = allUnits.where((u) => u['vehicle_status'].toString().toLowerCase() == 'dispatched').length;
      ...
      final reportsResponse = await _supabase.from('incidents_reports').select('ticket_id, ticket_status, ticket_date_created, ticket_incidents_type').gte('ticket_date_created', startOfYearEpoch);
  ...
  String _getMunicipalityName(String zipCode) {
    switch (zipCode) {
      ...
      case "0000": return "Provincial Command";
      default: return "Municipality ($zipCode)";
    }
  }
  ...
  Widget _buildMobileLayout() {
    return Scaffold(backgroundColor: bgColor, appBar: _buildAppBar(), body: _buildDashboardContent(crossAxisCount: 2));
  }

  Widget _buildTabletLayout() {
    return Scaffold(backgroundColor: bgColor, appBar: _buildAppBar(), body: _buildDashboardContent(crossAxisCount: 3));
  }
  ...
  AppBar _buildAppBar({bool isDesktop = false}) {
    return AppBar(
      ...
      centerTitle: false,
      title: const Text("MDRRMO PANEL", style: TextStyle(fontWeight: FontWeight.w900, letterSpacing: -0.5, fontSize: 18)),
          ...
          Expanded(
            child: ListView(
              ...
              children: [
                ...
                _buildSidebarItem(Icons.satellite_alt_rounded, "Live Map", false, () => Navigator.push(context, MaterialPageRoute(builder: (_) => const MDRRMOLiveMap()))),
                _buildSidebarItem(Icons.campaign_rounded, "Emergency Broadcast", false, () => Navigator.push(context, MaterialPageRoute(builder: (_) => const MDRRMONotificationCenter()))),
  ...
  Widget _buildSidebarHeader() {
    return Padding(
      ...
      child: Row(
        children: [
          Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: emergencyRed.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(12)), child: Icon(Icons.shield_rounded, color: emergencyRed, size: 24)),
          const SizedBox(width: 12),
          const Text("MDRRMO", style: TextStyle(fontWeight: FontWeight.w900, fontSize: 20, letterSpacing: -0.5)),
        ],
      ),
    );
  }
    ...
    return Padding(
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(color: isActive ? emergencyRed.withValues(alpha: 0.1) : Colors.transparent, borderRadius: BorderRadius.circular(12)),
          child: Row(
            children: [
  ...
  Widget _buildSidebarFooter() {
    return Padding(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            CircleAvatar(radius: 18, backgroundImage: _avatarUrl != null ? NetworkImage(_avatarUrl!) : null, backgroundColor: lockedBgColor, child: _avatarUrl == null ? const Icon(Icons.person, size: 18) : null),
            const SizedBox(width: 12),
          ...
          child: ListView(
            ...
            children: [
              ...
              GridView.count(
                ...
                children: [
                  _buildStatCard("Total Personnel", _totalPersonnel.toString(), const Color(0xFF8B5CF6), Icons.groups_rounded),
                  _buildStatCard("Active Units", _activeUnits.toString(), const Color(0xFFF59E0B), Icons.fire_truck_rounded),
                  _buildStatCard("Pending Reports", _pendingReports.toString(), emergencyRed, Icons.report_problem_rounded),
                  _buildStatCard("Yearly Incidents", _yearlyIncidents.toString(), const Color(0xFF10B981), Icons.analytics_rounded),
                ],
              ),

              const SizedBox(height: 32),

              _buildSectionTitle(Icons.auto_graph_rounded, "VISUAL PERFORMANCE", primaryDark),
              const SizedBox(height: 16),
              _buildActivityBarChart(),

              const SizedBox(height: 32),

              _buildSectionTitle(Icons.pie_chart_rounded, "INCIDENT BREAKDOWN", primaryDark),
              const SizedBox(height: 16),

              LayoutBuilder(builder: (context, constraints) {
                return Responsive.isMobile(context)
                    ? Column(children: [_buildIncidentTypeBreakdown(), const SizedBox(height: 16), _buildMonthlyTrendChart()])
                    : Row(children: [Expanded(flex: 2, child: _buildIncidentTypeBreakdown()), const SizedBox(width: 24), Expanded(flex: 3, child: _buildMonthlyTrendChart())]);
              }),

              const SizedBox(height: 32),

              if (!isDesktop) ...[
                _buildSectionTitle(Icons.bolt_rounded, "ACTIVE MODULES", emergencyRed),
                const SizedBox(height: 16),
                GridView.count(
                  ...
                  children: [
                    _buildGridCard(title: "Emergency Broadcast", subtitle: "Send instant alerts to citizens.", icon: Icons.campaign_rounded, iconColor: emergencyRed, isActive: true, onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const MDRRMONotificationCenter()))),
                    _buildGridCard(title: "Unit Dispatch", subtitle: "$_activeUnits active response fleets.", icon: Icons.fire_truck_rounded, iconColor: const Color(0xFFF59E0B), isActive: true, onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => UnitDispatch(municipalZipCode: _municipalityZipCode)))),
                    _buildGridCard(title: "Incident Reports", subtitle: "$_pendingReports pending reports.", icon: Icons.report_problem_rounded, iconColor: const Color(0xFF3B82F6), isActive: true, onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const IncidentReports()))),
                    _buildGridCard(title: "Personnel", subtitle: "$_totalPersonnel registered personnel.", icon: Icons.groups_rounded, iconColor: const Color(0xFF8B5CF6), isActive: true, onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => MdrrmoPersonnelList(mdrrmoMunicipality: _municipalityZipCode)))),
                    _buildGridCard(title: "Barangay Responders", subtitle: "Manage barangay registered personnel.", icon: Icons.badge_rounded, iconColor: const Color(0xFF0D9488), isActive: false, onTap: null),
                  ],
                ),
                const SizedBox(height: 32),
              ],
              ...
              GridView.count(
                ...
                childAspectRatio: isDesktop ? 1.2 : 0.85,
                children: [
                  _buildGridCard(title: "Evacuation Centers", subtitle: "Manage site statuses.", icon: Icons.house_siding_rounded, iconColor: const Color(0xFF10B981), isActive: false, onTap: null),
                ],
              ),
  ...
  Widget _buildStatCard(String title, String value, Color color, IconData icon) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: color.withValues(alpha: 0.1), shape: BoxShape.circle), child: Icon(icon, color: color, size: 20)), Text(value, style: TextStyle(fontSize: 24, fontWeight: FontWeight.w900, color: textPrimary))]),
        const Spacer(),
        Text(title, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: textSecondary, letterSpacing: 0.2)),
      ]),
    );
  }

  Widget _buildActivityBarChart() {
    double maxVal = _weeklyIncidentCounts.reduce((curr, next) => curr > next ? curr : next);
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text("Weekly Incidents", style: TextStyle(fontWeight: FontWeight.w800, fontSize: 13, color: Color(0xFF1E293B))), Icon(Icons.more_horiz_rounded, size: 18, color: textSecondary)]),
            ...
            child: BarChart(
              BarChartData(
                ...
                titlesData: FlTitlesData(
                    ...
                    sideTitles: SideTitles(
                      ...
                      getTitlesWidget: (value, meta) {
                        ...
                        return Padding(
                          ...
                          child: Text(days[value.toInt()], style: TextStyle(color: textSecondary, fontWeight: FontWeight.w700, fontSize: 10)),
                        );
                      },
                      reservedSize: 28,
                    ),
                  ...
                  leftTitles: AxisTitles(
                    sideTitles: SideTitles(
                      ...
                      interval: maxY / 5,
                      getTitlesWidget: (value, meta) {
                        return Text(value.toInt().toString(), style: TextStyle(color: textSecondary, fontWeight: FontWeight.w700, fontSize: 10));
                      },
                    ),
                  ),
                ),
                ...
                barGroups: List.generate(7, (i) {
                  final colors = [const Color(0xFF8B5CF6), const Color(0xFFF59E0B), emergencyRed, const Color(0xFF10B981), const Color(0xFF3B82F6), const Color(0xFF8B5CF6), const Color(0xFF64748B)];
                  return BarChartGroupData(
                    ...
                    barRods: [
                      BarChartRodData(
                        ...
                        backDrawRodData: BackgroundBarChartRodData(show: true, toY: maxY, color: bgColor),
                      ),
                    ],
                  );
                }),
  ...
  Widget _buildIncidentTypeBreakdown() {
    final sortedTypes = _incidentTypeBreakdown.entries.toList()..sort((a, b) => b.value.compareTo(a.value));
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: surfaceColor, borderRadius: BorderRadius.circular(24), border: Border.all(color: borderColor)),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text("Emergency Types", style: TextStyle(fontWeight: FontWeight.w900, fontSize: 14, color: Color(0xFF0F172A))),
        const SizedBox(height: 16),
        if (topTypes.isEmpty) const Expanded(child: Center(child: Text("No Data Available", style: TextStyle(fontSize: 12, color: Colors.grey))))
        else Expanded(
          child: ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              final double percent = _yearlyIncidents > 0 ? type.value / _yearlyIncidents : 0;
              return Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(type.key, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w700)), Text("${type.value}", style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w900))]),
                  const SizedBox(height: 6),
                  LinearProgressIndicator(value: percent, backgroundColor: bgColor, color: _getCategoryColor(type.key), minHeight: 4, borderRadius: BorderRadius.circular(2)),
                ],
              );
            },
          ),
  ...
  Widget _buildMonthlyTrendChart() {
    double maxVal = _monthlyIncidentCounts.reduce((curr, next) => curr > next ? curr : next);
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              const Text("Monthly Trend", style: TextStyle(fontWeight: FontWeight.w800, fontSize: 13, color: Color(0xFF0F172A))),
              Text(DateTime.now().year.toString(), style: TextStyle(fontSize: 12, fontWeight: FontWeight.w800, color: textSecondary)),
            ],
          ),
          const SizedBox(height: 24),
          Expanded(
                  ...
                  bottomTitles: AxisTitles(
                    sideTitles: SideTitles(
                      ...
                      getTitlesWidget: (value, meta) {
                        ...
                        return Padding(
                          padding: const EdgeInsets.only(top: 8.0),
                          child: Text(months[value.toInt()], style: TextStyle(color: textSecondary, fontWeight: FontWeight.w700, fontSize: 9)),
                        );
                      },
                      reservedSize: 28,
                    ),
                  ...
                  return BarChartGroupData(
                    ...
                    barRods: [
                      BarChartRodData(
                        ...
                        color: primaryDark.withValues(alpha: 0.2 + (_monthlyIncidentCounts[i] / maxY * 0.8)),
                        width: 8,
  ...
  Widget _buildInternalTestWarningCard() {
    return Container(
      ...
      decoration: BoxDecoration(color: warningAmber.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(16), border: Border.all(color: warningAmber.withValues(alpha: 0.3), width: 1.2)),
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Icon(Icons.science_rounded, color: warningAmber, size: 24),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text("INTERNAL TESTING PHASE", style: TextStyle(fontSize: 11, fontWeight: FontWeight.w900, color: warningAmber, letterSpacing: 1.0)), const SizedBox(height: 4), Text("This application is currently in a closed internal testing phase and is not yet ready for public distribution.", style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: primaryDark, height: 1.4))])),
      ]),
    );
  }
  ...
  Widget _buildNationalFleetTrackerCard() {
    return Container(
      decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFF0F172A), Color(0xFF1E293B)], begin: Alignment.topLeft, end: Alignment.bottomRight), borderRadius: BorderRadius.circular(20), boxShadow: [BoxShadow(color: primaryDark.withValues(alpha: 0.25), blurRadius: 12, offset: const Offset(0, 6))]),
      child: Material(
        color: Colors.transparent,
        child: InkWell(
          onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const MDRRMOLiveMap())),
          borderRadius: BorderRadius.circular(20),
          child: Padding(
            padding: const EdgeInsets.symmetric(horizontal: 24.0, vertical: 24.0),
            child: Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
              Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: const Color(0xFF10B981).withValues(alpha: 0.15), borderRadius: BorderRadius.circular(14)), child: const Icon(Icons.satellite_alt_rounded, color: Color(0xFF10B981), size: 32)),
              const SizedBox(width: 24),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
                Row(children: [Container(width: 6, height: 6, decoration: const BoxDecoration(color: Color(0xFF10B981), shape: BoxShape.circle)), const SizedBox(width: 6), const Text("LIVE MAP", style: TextStyle(fontSize: 10, fontWeight: FontWeight.w900, color: Color(0xFF10B981), letterSpacing: 1.0))]),
                const SizedBox(height: 4),
                const Text("INCIDENTS MONITORING", style: TextStyle(fontSize: 22, fontWeight: FontWeight.w800, color: Colors.white, letterSpacing: -0.5), maxLines: 1, overflow: TextOverflow.ellipsis),
                const SizedBox(height: 4),
                const Text("Monitor all active response units across the province. This is shared across all registered municipalities.", style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: Colors.white70, height: 1.3), maxLines: 2, overflow: TextOverflow.ellipsis),
              ])),
              const SizedBox(width: 12),
              Icon(Icons.arrow_forward_ios_rounded, color: Colors.white.withValues(alpha: 0.3), size: 20),
            ]),
          ),
        ),
      ),
  ...
  Widget _buildAdminProfileHeader() {
    return Container(
      ...
      decoration: BoxDecoration(color: surfaceColor, borderRadius: BorderRadius.circular(20), border: Border.all(color: borderColor), boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: 0.02), blurRadius: 10, offset: const Offset(0, 4))]),
      child: Row(children: [
        Container(height: 60, width: 60, decoration: BoxDecoration(color: lockedBgColor, shape: BoxShape.circle, border: Border.all(color: borderColor), image: _avatarUrl != null ? DecorationImage(image: NetworkImage(_avatarUrl!), fit: BoxFit.cover) : null), child: _avatarUrl == null ? Icon(Icons.person_rounded, size: 30, color: textSecondary.withValues(alpha: 0.5)) : null),
        const SizedBox(width: 16),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text("ADMINISTRATOR", style: TextStyle(fontSize: 10, fontWeight: FontWeight.w900, color: emergencyRed, letterSpacing: 1.5)),
          ...
          Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4), decoration: BoxDecoration(color: emergencyRed.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(12), border: Border.all(color: emergencyRed.withValues(alpha: 0.2))), child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.shield_rounded, size: 12, color: emergencyRed), const SizedBox(width: 4), Text("MDRRMO • $_municipalityName", style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: emergencyRed, letterSpacing: 0.5))])),
        ])),
      ]),
    );
  }
